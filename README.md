...
**QR 흐름 요약**: 참가자 QR 스캔 → `ScanLogs` 시트에 기록 → `자동출석트리거` Action 실행 → `설문지 응답 시트1`의 `출석 상태`(Q열)를 `"Y"`로 업데이트 → `code.gs`가 참석자 구분
...
2. **Type**: **`Yes/No`**를 선택합니다.
3. **Editable**: 체크박스를 **켭니다(ON)**.
4. **[Done]**을 클릭합니다.

> ⚠️ **중요**: `출석 상태`는 반드시 **Yes/No** 타입으로 설정하세요. 이후 Action에서 값을 `"Y"` (문자열)로 쓰는데, 이 타입 조합이어야 `code.gs`의 `sendPostEventEmails()`가 참석자(`Y`)와 불참자(`N`)를 올바르게 구분합니다.
...
1. `스캔된이메일` 컬럼의 연필 아이콘을 클릭합니다.
2. **Type**: `Text`
3. **Initial value**에 아래 수식을 입력합니다. QR에서 스캔된 `이름|이메일` 문자열에서 `|` 뒤의 이메일 부분을 자동으로 추출합니다.

   ```text
   INDEX(SPLIT([스캔된성명], "|"), 2)
   ```

4. **Editable**: **끕니다(OFF)** — 사용자가 직접 수정하지 못하게 합니다.
5. **[Done]**을 클릭합니다.

##### 참가자찾기 컬럼 추가 (Virtual Ref 컬럼 — 핵심)

이 컬럼은 스캔된 이름과 이메일로 `설문지 응답 시트1`에서 해당 참가자를 찾아주는 "링크" 역할을 합니다.

1. `ScanLogs` 컬럼 목록 하단의 **[+ Add virtual column]** 버튼을 클릭합니다.
...
5. **App formula**에 아래 수식을 입력합니다.
   ...
   ANY(
     SELECT(설문지 응답 시트1[이름],
       AND(
         [이름] = INDEX(SPLIT([스캔된성명], "|"), 1),
         [이메일] = INDEX(SPLIT([스캔된성명], "|"), 2)
       )
     )
   )
   ```

6. **Show**: **끕니다(OFF)** — 사용자 화면에 보이지 않게 합니다.
7. **[Done]**을 클릭합니다.

> ℹ️ **수식 설명**: `SPLIT([스캔된성명], "|")`는 `홍길동|hong@example.com`을 `["홍길동", "hong@example.com"]`으로 분리합니다. `INDEX(..., 1)`은 이름, `INDEX(..., 2)`는 이메일을 가져와 `설문지 응답 시트1`에서 이름과 이메일이 모두 일치하는 참가자를 찾습니다.
...
| 스캔된성명 | Text | — | — | **Scannable ON** |
| 스캔된이메일 | Text | `INDEX(SPLIT([스캔된성명], "\|"), 2)` | — | Editable OFF |
...
4. **[Add]** 버튼(Column Inputs 섹션의 **[+]**)을 클릭하여 변경할 컬럼을 추가합니다.
   * 컬럼 선택: **`출석 상태`**
   * 값(Value): **`"Y"`** (큰따옴표 포함하여 입력)
5. **Position**: `Hide`를 선택합니다. (앱 화면에 버튼으로 표시하지 않음)
6. **[Done]**을 클릭합니다.

> ⚠️ 값을 입력할 때 반드시 **`"Y"`** (큰따옴표 포함)로 입력하세요. `TRUE`, `1`, `Y`(따옴표 없음)로 입력하면 사후 메일 발송 스크립트가 출석자를 인식하지 못합니다.
...
> ℹ️ **동작 흐름**: QR 스캔 → `ScanLogs`에 새 행 추가 → `자동출석트리거` 실행 → `참가자찾기`로 해당 참가자를 `설문지 응답 시트1`에서 찾음 → `출석상태변경` Action이 그 참가자의 `출석 상태`를 `"Y"`로 변경

---

### 5단계: 뷰(화면) 설정 — 스캐너 화면 만들기

AppSheet는 데이터 입력용 화면(뷰)을 자동으로 만들어 줍니다. 여기서는 QR 스캔 후 자동으로 다음 스캔 화면으로 돌아오도록 설정합니다.

1. 좌측 메뉴 **[UX]** → **[Views]**를 클릭합니다.
2. 목록에서 **`ScanLogs_Form`** 뷰를 클릭합니다. (없으면 **[+ New View]** → View type: `Form` → Data: `ScanLogs` 선택)
3. 뷰 이름을 **`스캐너`**로 변경해도 좋습니다. (선택 사항)
4. **[Behavior]** 섹션을 찾아 펼칩니다.
   * **Event Actions** → **Form Saved** 드롭다운에서 **`자동출석트리거`**를 선택합니다.
   * 이 설정이 핵심입니다: 스캔이 저장될 때마다 자동으로 출석 Action을 실행합니다.
5. **[Navigation]** 섹션을 찾아 펼칩니다.
   * **Finish view**: **`스캐너`** (또는 `ScanLogs_Form`)를 선택합니다.
   * 스캔 저장 후 같은 화면으로 돌아와 다음 참가자를 연속으로 스캔할 수 있습니다.
6. **[Save]**를 클릭합니다.
...
2. **[스캔된성명]** 입력창을 탭합니다. 오른쪽에 QR 코드 모양 아이콘(📷)이 보여야 합니다.
3. 아이콘을 탭하면 카메라가 켜집니다. 참가자가 이메일로 받은 QR 코드를 카메라로 비춥니다.
4. QR 스캔이 되면 `스캔된성명` 필드에 `홍길동|hong@example.com` 형식의 텍스트가 자동 입력됩니다.
5. **[Save]** 버튼을 탭합니다.
6. 구글 스프레드시트 `설문지 응답 시트1`에서 해당 참가자의 Q열(`출석 상태`)이 **Y**로 바뀌었는지 확인합니다.
...
> | `참가자찾기`가 빈값 | App formula 수식 오타 | 수식에 `\|` 파이프 문자가 맞는지 확인 |
> | QR 아이콘이 안 보인다 | PC 브라우저 사용 중 | 스마트폰 앱에서만 표시됨 (정상) |
> | `출석 상태`가 Y 대신 TRUE로 기록 | Action 값 설정 오류 | Action 1에서 값을 `"Y"` (큰따옴표 포함)로 입력 |
> | 앱이 목록에 안 보인다 | 다른 계정으로 로그인 중 | 앱을 만든 구글 계정으로 다시 로그인 |

---

> 💡 **운영 흐름 전체**: 행사 전날 `sendFinalNoticeWithQR` 실행 → Q열 `"N"` 초기화 + QR 메일 발송 → 행사 당일 AppSheet QR 스캔 → Q열 `"Y"` 업데이트 → 행사 후 `사후 감사/안내 메일 발송` 실행 → Q열 값 기준으로 참석자/불참자 자동 분류 발송
