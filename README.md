...
QR 스캔 현황을 차트로 보여주는 뷰입니다. 이 뷰는 홈 대시보드에 삽입됩니다.
...
3. **View Options** 섹션의 **Chart type**에서 아래 중 하나를 선택합니다.

   | 차트 종류 | 설명 | 추천 상황 |
   | --- | --- | --- |
   | `aggregate donutchart` | 출석(Y)/미출석(N) 비율을 도넛 형태로 표시 | 비율 파악에 직관적 — **권장** |
   | `aggregate piechart` | 같은 내용을 파이 형태로 표시 | donut과 동일, 취향에 따라 선택 |
   | `col series` | Y/N 각 인원 수를 세로 막대로 비교 | 절대 인원 수 비교가 필요할 때 |

   > ℹ️ AppSheet에는 "Bar" 차트가 없습니다. 막대 형태는 `col series`, 비율 형태는 `aggregate donutchart`를 선택하세요.

4. **Group aggregate** 설정:
   * **Group by** (또는 **Chart columns**): `출석 상태` 선택
   * **Aggregate**: `COUNT`

5. **Position**: `middle` 또는 원하는 위치를 선택합니다. 하단 탭에 단독으로 노출하려면 `first` / `next` 중 하나를, 대시보드에만 포함할 경우 `ref`를 선택합니다.
6. **[Done]**을 클릭합니다.

> ℹ️ **차트 읽는 법**: 출석(Y) 영역 = 현재까지 QR 스캔된 참가자 수, 미출석(N) 영역 = 아직 스캔되지 않은 인원. 전체 합 = 총 신청자 수입니다.
