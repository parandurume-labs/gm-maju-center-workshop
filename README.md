...
QR 스캔 현황을 막대 그래프로 보여주는 뷰입니다. 이 뷰는 홈 대시보드에 삽입됩니다.
...
3. **Chart** 설정 섹션에서 아래와 같이 입력합니다.

   | 항목 | 설정값 | 설명 |
   | --- | --- | --- |
   | Chart type | `Bar` | 출석(Y)/미출석(N) 막대 비교 |
   | Group by | `출석 상태` | Y / N 두 그룹으로 분류 |
   | Aggregate | `COUNT` | 각 그룹의 행 수를 세어 표시 |
   | Label | `출석 현황 (실시간)` | 차트 제목 |

4. **Show in**: `Do not show` — 이 뷰는 대시보드 안에 포함할 것이므로 하단 탭에는 표시하지 않습니다.
5. **[Done]**을 클릭합니다.

> ℹ️ **차트 읽는 법**: Y 막대 = 현재까지 QR 스캔된 참가자 수, N 막대 = 아직 미출석 인원. 두 막대를 합하면 총 신청자 수입니다.
